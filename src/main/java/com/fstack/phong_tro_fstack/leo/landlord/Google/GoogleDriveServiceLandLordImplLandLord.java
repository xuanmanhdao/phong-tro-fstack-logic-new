...
public class GoogleDriveServiceLandLordImplLandLord implements GoogleDriveServiceLandLord {
  ...
  private Drive getDrive() throws IOException {
    Credential credential = GoogleCredential.fromStream(serviceAccountKey.getInputStream())
        .createScoped(SCOPES);
...
//        gDriveFile.setParents(Collections.singletonList(folderId));
  ...
  public List<FileResponse> getAllFile() throws IOException {
    ...
    FileList fileList = drive.files().list()
        .setFields("files(id,name,thumbnailLink,webViewLink,webContentLink)").execute();
